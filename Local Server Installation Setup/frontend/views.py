...
@login_required(login_url='/')
def syncNumberPipeDataJSON(request):
    machine_list = Machine.objects.filter(user=request.user).values_list('machine_id', flat=True)
    try:
        return JsonResponse({
            "syncNmber": PipeData.objects.filter(mid__in=machine_list, synced=False).count()
        }, status=200)
    except Exception as ex:
        return JsonResponse({
            "error": str(ex)
...
@login_required(login_url='/')
def syncPipeDataJSON(request):
    try:
        machine_list = Machine.objects.filter(user=request.user).values_list('machine_id', flat=True)
        from api.local_to_global_sync import sync_method
        sncpd = sync_method(machine_list)
        return JsonResponse({
           "syncPipeData": sncpd
        }, status=200)
    except Exception as ex:
...
@login_required(login_url='/')
def changeSyncMachineJSON(request):
    try:
        sync_manually = request.GET.get('sync_manually')
        if sync_manually == "true":
            sync_manually = True
        else:
            sync_manually = False
        Machine.objects.filter(user=request.user).update(sync_manually=sync_manually)
