...
def summary_1_view(request):
    ...
    try:
        total_pipe = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list).count()
    except:
        total_pipe = None
    ...
    totalpipecount = {}
    try:
        totalpipecount["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1').count()
        totalpipecount["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2').count()
        totalpipecount["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3').count()
        totalpipecount["total"] = total_pipe
    except:
        totalpipecount = None
    passed = {}
    try:
        passed["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1', pass_status='Passed').count()
        passed["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2', pass_status='Passed').count()
        passed["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3', pass_status='Passed').count()
    ...
    overweight = {}
    try:
        overweight["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1', pass_status='Overweight').count()
        overweight["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2', pass_status='Overweight').count()
        overweight["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3', pass_status='Overweight').count()
    ...
    totalweight = {}
    try:
        totalweight["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1').aggregate(Sum('weight'))['weight__sum']
        totalweight["shift1"] = 0 if totalweight["shift1"] == None else totalweight["shift1"] / 1000
        totalweight["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2').aggregate(Sum('weight'))['weight__sum'] 
        totalweight["shift2"] = 0 if totalweight["shift2"] == None else totalweight["shift2"] / 1000
        totalweight["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3').aggregate(Sum('weight'))['weight__sum'] 
    ...
    length = {}
    try:
        length["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1').aggregate(Sum('length'))['length__sum']
        length["shift1"] = 0 if length["shift1"] == None else length["shift1"]
        length["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2').aggregate(Sum('length'))['length__sum']
        length["shift2"] = 0 if length["shift2"] == None else length["shift2"]
        length["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3').aggregate(Sum('length'))['length__sum']
    ...
    weightgain = {}
    try:
        weightgain["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1').aggregate(Sum('weightgain'))['weightgain__sum'] 
        weightgain["shift1"] = 0 if weightgain["shift1"] == None else weightgain["shift1"] / 1000
        weightgain["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2').aggregate(Sum('weightgain'))['weightgain__sum']
        weightgain["shift2"] = 0 if weightgain["shift2"] == None else weightgain["shift2"] / 1000
        weightgain["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3').aggregate(Sum('weightgain'))['weightgain__sum']
    ...
    weightloss = {}
    try:
        weightloss["shift1"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='1').aggregate(Sum('weightloss'))['weightloss__sum']
        weightloss["shift1"] = 0 if weightloss["shift1"] == None else weightloss["shift1"] / 1000
        weightloss["shift2"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='2').aggregate(Sum('weightloss'))['weightloss__sum']
        weightloss["shift2"] = 0 if weightloss["shift2"] == None else weightloss["shift2"] / 1000
        weightloss["shift3"] = PipeDataProcessed.objects.filter(timestamp__gte = starttimestamp, timestamp__lte = endtimestamp, machine_id__in = machine_list, shift='3').aggregate(Sum('weightloss'))['weightloss__sum']
